...
We should noe be able to list the entries in the new credential store

[source,sh,role="copypaste"]
----
$EAP7_HOME/elytron-tool.sh credential-store --location=$EAP7_HOME/standalone/configuration/my_vault_converted.cred_store --aliases
----
...
[source,sh,role="copypaste"]
----
 bin/elytron-tool.sh credential-store --location=standalone/configuration/converted.store --generate-secret-key secret
----

You will be prompted to enter the credential store password: enter "vault22"

We now need to add the credential store and expression resolver to the server.
...
Run the following to encrypt an existing entry in the credential store

[source,sh,role="copypaste"]
----
$EAP7_HOME/bin/elytron-tool.sh credential-store --location=standalone/configuration/my_vault_converted.cred_store --encrypt secret --entry vb::dbusername
----
...
[source,sh,role="copypaste"]
----
/subsystem=datasources/data-source=postgresql:undefine-attribute(name=username)
----

[source,sh,role="copypaste"]
----
/subsystem=datasources/data-source=ExampleDS:write-attribute(name=username, value="${ENC::RUxZAUMQ5BifXly3OOATJrn5wd3q5g411eHS3tZz516OTfmHEIE=}")
----

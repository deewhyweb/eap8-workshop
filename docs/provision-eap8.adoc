...
We can now add use this token as our user-name
...
Run the following command to use the token for user-name, replacing "xxxx" with the token produced previous step
[source,sh,role="copypaste"]
----

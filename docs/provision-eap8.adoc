...
Run the following command to migrate the legacy vault to a credential store called "my_vault_converted.cred_store" located in "$EAP7_HOME/standalone/configuration/".
...
We should now be able to list the entries in the new credential store
...
We now need to add the credential store and expression resolver to the server. Login to the via the JBoss CLI
...
Create a credential store called exampleKeyStoreCredentialStore using the credential store "my_vault_converted.cred_store" located in "$EAP7_HOME/standalone/configuration/"(jboss.server.config.dir)
...
Define an expression resolver using the secret we created previously.
...
Test the credential store aliases
...
Remove the password attribure from the postgresql data-source
...
Create a credential-reference attribute for the postgresql data-source to provide the password form the exampleKeyStoreCredentialStore credential store.
...
Run the following to create an ecrypted token from the value of vb::dbusername in the credential store.  
...
We can now add use this token as our user-name attribute on the postgresql data-source.

Login to the JBoss CLI
...
Remove the previous user-name attribute from the postresql data-source
...
Run the following command to use a token for user-name, replacing "xxxx" with the token produced previous step.
...
Remove the legacy vault from JBoss EAP 7.4.0
...
Reload the JBoss configuration

...
&& export POSTGRESQL_DATASOURCE=CoolstoreDS \
...
[source,sh,role="copypaste"]
----
./target/server/bin/standalone.sh -c standalone-full-ha.xml -Djboss.http.port=8100  -b 0.0.0.0
----
...
We can now push our code changes to Git.

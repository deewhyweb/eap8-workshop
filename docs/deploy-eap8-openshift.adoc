...
*Step 1:* First step is to add the Red Hat plugin repositories for GA and early access software. Open the file `coolstore-eap8/pom.xml` and add this after the repositories tag.
...
*Step 2:* Also in the pom.xml file and add the following before the closing tag replacing the existing <profiles> block:
...
*Step 3:* Click on the "Source Control" icon in the IDE, you should see the following:

image::gitea-eap8-ocp.png[source-contol]

*Step 4:* Enter a commit message in the message field and click on the "Commit" button.

You will be prompted to stage files to the commit

image::git-2.png[stage-files]

*Step 5:* Click on "Yes" and then click on "Sync Changes" to push your changes to gitea

You will be prompted to enter a username:

image::git-3.png[git-username]

*Step 6:* Enter `{{ USER_ID }}`

You will then be prompted for a password:

*Step 7:* Enter `{{ CHE_USER_PASSWORD }}`

Your changes will be pushed to gitea.

We can now deploy our {{ EAP8_VERSION }} application with Helm.

*Step 8:* Access the OpenShift console by clicking on the following {{ CONSOLE_URL }}[link^] 
...
*Step 9:* Switch to the project called "{{ USER_ID }}-project"

*Step 10:* Click on Add+.

*Step 11:* Select Helm Chart from the developer catalog.

*Step 12:* Enter eap in the Filter by keyword field as shown below:

image::helm-charts.png[migration,80%]

*Step 13:* Select the JBoss EAP 8 Helm chart from the catalog.

*Step 14:* Click on "Create".

*Step 16:* Switch to YAML view.

*Step 17:* Delete the existing content.

*Step 18:* Paste the following YAML:
...
*Step 19:* Go to Builds. 

image::eap8-builds.png[migration,80%]

*Step 20:* Select eap8-build-artifacts.

*Step 21:* Choose the Builds tab.

*Step 22:* Click on eap8-build-artifacts-1 to view the logs of the active build.

*Step 23:* Once the builds are complete, you will be able to click the Open URL icon of the eap8 deployment to view the running application.
